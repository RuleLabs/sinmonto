...
Ordre d'insertion stable, implémenté via le tri stable garanti par Python (`sorted(rules, key=priority, reverse=True)`). `DecisionTrace.evaluation_order` enregistre l'ordre exact d'évaluation de chaque cycle, pour audit. Garantie documentée : entrées identiques + ordre d'enregistrement identique ⇒ sorties bit-à-bit identiques, indépendamment de l'environnement d'exécution — **à l'exception explicite de `DecisionTrace.trace_id`**, généré via `uuid.uuid4()` et donc non reproductible par nature. La garantie de déterminisme porte sur les règles matchées, les effets produits, l'ordre d'évaluation, l'état du contexte et les résultats de condition — pas sur les identifiants générés à l'évaluation. Zone grise trouvée et signalée en revue croisée (Qwen, puis ChatGPT sur ce tour) ; non corrigée par un `trace_id` déterministe (`uuid5` sur `signal_id`/`entity_id`/`context_version` serait l'option si ça devient nécessaire) — clarifiée en documentation pour cette preview plutôt que changée en code, 2026-08.
